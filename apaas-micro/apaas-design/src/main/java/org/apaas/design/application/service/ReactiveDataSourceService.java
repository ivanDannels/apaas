...
public interface ReactiveDataSourceService extends ApplicationService<DataSource, Long> {
     ...
     * 分页查询数据源
     */
    Mono<PageResult<DataSourceDTO>> selectPage(Query query);
    
    /**
     * 创建数据源
     */
    Mono<DataSourceDTO> create(DataSourceDTO dataSourceDto);
    
    /**
     * 更新数据源
     */
    Mono<DataSourceDTO> update(Long id, DataSourceDTO dataSourceDto);
    
    /**
     * 根据ID获取数据源
     */
    Mono<DataSourceDTO> findById(Long id);
     ...
     * 保存数据源
     */
    Mono<DataSourceDTO> save(DataSourceDTO dataSourceDto);
