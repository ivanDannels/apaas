 ...
 */
package org.apaas.integration.interfaces.rest;
...
public class IntegrationRest extends BaseRest<IntegrationDTO, Long, IntegrationService> {
    
    public IntegrationRest(IntegrationService service) {
        super(service);
    }
    ...
    @GetMapping("/{id}")
    public Mono<IntegrationDTO> getIntegrationById(@PathVariable Long id) {
    ...
    public Mono<IntegrationDTO> createIntegration(@RequestBody IntegrationDTO integrationDTO) {
        return service.save(integrationDTO);
    }
    
    @Log(title = "更新集成", businessType = BusinessType.UPDATE, operatorType = OperatorType.MANAGE)
    @PutMapping("/{id}")
    public Mono<IntegrationDTO> updateIntegration(@Parameter(description = "集成ID", required = true) @PathVariable Long id, @RequestBody IntegrationDTO integrationDTO) {
        return service.save(integrationDTO);
    }
    ...
    public Flux<IntegrationDTO> findByType(@Parameter(description = "集成类型", required = true) @PathVariable String type) {
