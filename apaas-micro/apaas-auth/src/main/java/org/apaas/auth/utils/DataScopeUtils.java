...
public class DataScopeUtils {
    ...
    public static List<Long> getDeptIdsByDataScope(DataScope dataScope, List<Long> customDeptIds) {
        switch (dataScope) {
            ...
            case CURRENT_DEPT:
                // 查看当前部门数据
                Long currentDeptId = SecurityUtils.getCurrentUser().getDeptId();
                return currentDeptId != null ? List.of(currentDeptId) : List.of();
            case CURRENT_USER:
                // 查看个人数据，通过用户ID过滤，这里返回空列表
                return List.of();
            case CUSTOM_DEPTS:
                // 查看指定部门数据
                return CollectionUtils.isEmpty(customDeptIds) ? List.of() : customDeptIds;
            default:
                return List.of();
        }
    }
    ...
    public static String buildDataScopeSql(DataScope dataScope, List<Long> deptIds, Long userId,
        ...
        return sql.length() > 0 ? sql.substring(5) : "";
    }
}