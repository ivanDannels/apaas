...
class FlowDefinitionAssemblerTest {
    ...
    void testToDTO() {
        // 创建实体对象
        FlowDefinition entity = FlowDefinition.builder().id(1L).name("测试流程").description("测试流程描述").version(1).definition("流程定义内容").status(1).createdBy("admin").createdTime(LocalDateTime.now()).updatedBy("admin").updatedTime(LocalDateTime.now()).build();
    ...
    void testToDTOList() {
        // 创建实体列表
        List<FlowDefinition> entities = Arrays.asList(FlowDefinition.builder().id(1L).name("流程1").build(), FlowDefinition.builder().id(2L).name("流程2").build());
    ...
    void testToEntityList() {
        // 创建DTO列表
        List<FlowDefinitionDTO> dtos = Arrays.asList(new FlowDefinitionDTO() {
