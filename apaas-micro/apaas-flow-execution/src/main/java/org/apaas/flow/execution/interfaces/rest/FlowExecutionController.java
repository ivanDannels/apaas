 ...
 */
package org.apaas.flow.execution.interfaces.rest;
...
public class FlowExecutionController {
    ...
    public Mono<Result<FlowInstance>> startProcessInstance(@RequestBody StartProcessDTO startProcessDTO) {
        return flowRuntimeService.startProcessInstance(startProcessDTO.getProcessId(), startProcessDTO.getBusinessKey(), startProcessDTO.getStarter()).onErrorReturn(Result.error("启动流程实例失败"));
    }
    ...
    public Mono<Result<WorkflowTask>> completeTask(@RequestBody CompleteTaskDTO completeTaskDTO) {
        return taskManagementService.completeTask(completeTaskDTO.getTaskId(), completeTaskDTO.getUserId()).onErrorReturn(Result.error("完成任务失败"));
    }
    ...
    public Mono<Result<WorkflowTask>> transferTask(@RequestBody TransferTaskDTO transferTaskDTO) {
        return taskManagementService.transferTask(transferTaskDTO.getTaskId(), transferTaskDTO.getFromUserId(), transferTaskDTO.getToUserId()).onErrorReturn(Result.error("转办任务失败"));
    }
}