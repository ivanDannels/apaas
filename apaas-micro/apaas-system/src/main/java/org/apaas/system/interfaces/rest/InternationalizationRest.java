...
public class InternationalizationRest extends BaseRest<Internationalization, Long, ReactiveInternationalizationService> {
    
    public InternationalizationRest(ReactiveInternationalizationService service) {
        super(service);
    }
     ...
     * @return 消息列表
     */
    @GetMapping(value = "/messages/{language}", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "根据语言获取所有国际化消息", description = "根据语言获取所有国际化消息")
    @Parameter(name = "language", description = "语言代码，如zh_CN、en等", required = true)
    public Flux<InternationalizationDTO> getMessagesByLanguage(@PathVariable String language) {
        return service.getMessagesByLanguage(language)
                .map(InternationalizationAssembler.INSTANCE::convertEntityToDto);
    }
}